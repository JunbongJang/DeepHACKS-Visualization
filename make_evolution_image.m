...
%load the image
img = imread([imDir filesep imNames(1, 1).name]);
[row, col] = size(img);
...
fig = figure(2)   
imagesc(imgRGB), axis image, axis off;
windowingEvolution(movieData, windowing, time, clusters, duration_time, cluster_cmap, savePath, iWinProc, frame_list, replaced_path,root_path);
...
%%
function windowingEvolution(movieData, windowing, time, clusters, duration_time, cluster_cmap, savePath, iWinProc, frame_list, replaced_path,root_path) 
    for iFrame = frame_list
        %% Get the windows
